...
// TestSelectedServerDelay 测试当前选中的服务器延迟
func (pm *PingManager) TestSelectedServerDelay() (int, error) {
	// 获取当前选中的服务器
	selectedServer, err := pm.serverManager.GetSelectedServer()
	if err != nil {
		return -1, err
	}

	// 测试延迟
	delay, err := pm.TestServerDelay(*selectedServer)
	if err != nil {
		return -1, err
	}

	// 更新服务器延迟
	pm.serverManager.UpdateServerDelay(selectedServer.ID, delay)

	return delay, nil
}

// TestServerDelayByID 根据ID测试服务器延迟
func (pm *PingManager) TestServerDelayByID(id string) (int, error) {
	// 获取服务器
	s, err := pm.serverManager.GetServer(id)
	if err != nil {
		return -1, err
	}

	// 测试延迟
	delay, err := pm.TestServerDelay(*s)
	if err != nil {
		return -1, err
	}

	// 更新服务器延迟
	pm.serverManager.UpdateServerDelay(id, delay)
